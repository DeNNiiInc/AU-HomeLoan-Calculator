...
function calculate() {
    ...
    // Standard Repayment (P&I)
    const remainingTermPeriods = n - ioPeriods;
    let piRepayment = 0;
    if (remainingTermPeriods > 0) {
        piRepayment = (state.loanAmount * r * Math.pow(1 + r, remainingTermPeriods)) / (Math.pow(1 + r, remainingTermPeriods) - 1);
    }
    const ioRepayment = state.loanAmount * r;
    ...
    // Baseline
    let baseBalance = state.loanAmount;
    const baseBalances = [];
    ...
    for (let i = 0; i <= maxPeriods; i++) {
        ...
        if (i > 0) {
            ...
            const shouldAdd = state.tableMode === 'monthly' ? true : (i % state.freq === 0);
    ...
    document.getElementById('repaymentValue').textContent = fmt.format(currentRepayment);
    document.getElementById('totalInterest').textContent = fmt.format(totalInterest);

    const totalCost = state.loanAmount + totalInterest + state.stampDuty + state.otherFees + state.upfrontFees;
    document.getElementById('totalCost').textContent = fmt.format(totalCost);

    // Upfront Cash (includes deposit, legal costs, and all fees)
    const upfrontCash = state.depositAmount + state.legalCosts + state.stampDuty + state.otherFees + state.upfrontFees;
    document.getElementById('upfrontCash').textContent = fmt.format(upfrontCash);

    // Total Monthly Outgoings
    const monthlyExpenses = state.insurance + state.hoa + state.rates + state.otherMonthly;
    ...
    document.getElementById('totalMonthly').textContent = fmt.format(totalPeriodCost);

    const freqMap = { 12: 'Monthly', 26: 'Fortnightly', 52: 'Weekly' };
    document.getElementById('frequencyLabel').textContent = (state.isIO ? 'Interest Only (' : 'Principal & Interest (') + freqMap[state.freq] + ')';

    drawCharts(labels, balances, baseBalances, state.loanAmount, totalInterest, state.stampDuty + state.otherFees + state.upfrontFees);
...
function toggleTableMode() {
    ...
    document.getElementById('tableModeBtn').textContent = state.tableMode === 'yearly' ? 'Show Monthly' : 'Show Yearly';
    calculate();
}
...
function copyTable() {
    ...
    scheduleData.forEach(row => {
        csv += `${row.period},${row.balance.toFixed(2)},${row.interest.toFixed(2)},${row.principal.toFixed(2)},${row.extra.toFixed(2)}\n`;
    });
    navigator.clipboard.writeText(csv).then(() => alert('Table data copied to clipboard (CSV format)!'));
}
...
function drawCharts(labels, actualData, baseData, principal, interest, fees) {
    ...
    // Line Chart
    drawLineChart(balanceChartCtx, labels, actualData, baseData, colorGrid, colorText, colorBase, colorMain);

    // Update Balance Chart Legend
    updateBalanceLegend(labels, actualData, baseData);

    // Donut Chart
    drawDonutChart(breakdownChartCtx, principal, interest, fees, isDark, colorPrincipal, colorInterest, colorFees);

    // Update Breakdown Chart Legend
    updateBreakdownLegend(principal, interest, fees, colorPrincipal, colorInterest, colorFees);
}
...
function updateBalanceLegend(labels, actualData, baseData) {
    ...
    legend.innerHTML = `
        <div class="legend-item">
            ...
            <span>With Extras: <span class="legend-value">${fmt.format(finalActual)}</span></span>
        </div>
        <div class="legend-item">
            <div class="legend-color legend-dashed"></div>
            <span>Standard: <span class="legend-value">${fmt.format(finalBase)}</span></span>
        </div>
    `;
}
...
function updateBreakdownLegend(principal, interest, fees, colorP, colorI, colorF) {
    ...
    legend.innerHTML = `
        <div class="legend-item">
            ...
            <span>Principal: <span class="legend-value">${fmt.format(principal)} (${pPct}%)</span></span>
        </div>
        <div class="legend-item">
            <div class="legend-color" style="background: ${colorI};"></div>
            <span>Interest: <span class="legend-value">${fmt.format(interest)} (${iPct}%)</span></span>
        </div>
        ${fees > 0 ? `
        <div class="legend-item">
            <div class="legend-color" style="background: ${colorF};"></div>
            <span>Fees: <span class="legend-value">${fmt.format(fees)} (${fPct}%)</span></span>
        </div>` : ''}
    `;
}
...
function drawLineChart(ctx, labels, data1, data2, cGrid, cText, cBase, cMain) {
    ...
    for (let i = 0; i <= 5; i++) {
        ...
        const labelText = labelVal >= 1000 ? '$' + Math.round(labelVal / 1000) + 'k' : '$' + Math.round(labelVal);
    ...
    setupBalanceTooltip(canvas, labels, data1, data2, getX, getY, padding, chartW, chartH);
}

function setupBalanceTooltip(canvas, labels, actualData, baseData, getX, getY, padding, chartW, chartH) {
        ...
        if (index >= 0 && index < labels.length) {
            ...
            tooltipContent.innerHTML = `
                ...
                <div class="tooltip-row" style="margin-top: 0.5rem; padding-top: 0.5rem; border-top: 1px solid var(--border);">
                    ...
                    <span class="tooltip-value" style="color: var(--success);">${fmt.format(savings)}</span>
                </div>` : ''}
            `;
...
function drawDonutChart(ctx, principal, interest, fees, isDark, colorP, colorI, colorF) {
    const canvas = ctx.canvas;
    const width = canvas.width = canvas.offsetWidth;
    const height = canvas.height = canvas.offsetHeight;
    ctx.clearRect(0, 0, width, height);
            ...
            if (segment) {
                ...
                tooltipContent.innerHTML = `
                    <div class="tooltip-row">
                        ...
                        <span class="tooltip-value">${fmt.format(segment.value)}</span>
                    </div>
                    <div class="tooltip-row">
    ...
    list.innerHTML = scenarios.map(s => `
        <div class="scenario-item" onclick="loadScenario(${s.id})">
            <div class="scenario-info">
                <h4>${s.name}</h4>
                <p>${new Date(s.id).toLocaleDateString()} - $${s.data.loanAmount.toLocaleString()}</p>
            </div>
            <button class="delete-btn" onclick="deleteScenario(${s.id}, event)">✕</button>
        </div>
    `).join('');

    if (scenarios.length === 0) {
        list.innerHTML = '<div style="text-align:center; color:var(--text-sub); padding: 1rem;">No saved scenarios</div>';
    }
}
